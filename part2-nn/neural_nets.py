...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        vfun = np.vectorize(output_layer_activation_derivative)
        output_layer_error = - (y-activated_output) * vfun(output)# TODO
        vfun = np.vectorize(rectified_linear_unit_derivative)
        hidden_layer_error = output_layer_error * np.multiply(self.hidden_to_output_weights, vfun(hidden_layer_weighted_input).transpose())
        bias_gradients = hidden_layer_error.transpose()# TODO
        hidden_to_output_weight_gradients = (hidden_layer_activation@output_layer_error).transpose()# TODO
        input_to_hidden_weight_gradients = (input_values@hidden_layer_error).transpose()# TODO
    ...
    def predict(self, x1, x2):
        ...
        vfunc = np.vectorize(rectified_linear_unit)
        hidden_layer_activation = vfunc(hidden_layer_weighted_input)# TODO
        output = self.hidden_to_output_weights@hidden_layer_activation# TODO
        vfunc =np.vectorize(output_layer_activation)
        activated_output = vfunc(output)# TODO
